module axi_aw_handshake_unit_1 (
    input           i_clk,
    input           i_rst_n,
    ...
    always @(*) begin
        case (aw_state_r)
            ...
            `K_write : begin
                if (i_axi_awready && aw_cmd_cnt_r == `k_cache_aw_cnt) begin
                    aw_state_w = 1'b0;
                end
                else begin
                    aw_state_w = aw_state_r;
    ...
    always @(*) begin
        if (aw_state_r == `K_write && aw_cmd_cnt_r == `k_cache_aw_cnt && i_axi_awready) begin
            aw_cmd_cnt_w = 2'b0;
        end
        else if (aw_state_r == `K_write && i_axi_awready) begin
    ...
    always @(*) begin
        if (aw_state_r == `K_write && i_axi_awready && aw_cmd_cnt_r == `k_cache_aw_cnt) begin
    ...
    always @(*) begin
        if (aw_state_r == `K_write && i_axi_awready && aw_cmd_cnt_r == `k_cache_aw_cnt) begin
            ...
            o_axi_awaddr_w = `k_cache_start_addr + i_token_cnt[19:3] * `k_token_addr_spacing + i_layer_cnt * `k_cache_addr_spacing;
        end
        else if (aw_state_r == `K_write && i_axi_awready) begin
            o_axi_awaddr_w = o_axi_awaddr_r + `k_head_spacing;
        end
        else begin
...
//        .probe0(aw_state_r),
//        .probe1(aw_cmd_cnt_r),
//        .probe2(o_axi_awvalid_r),
//        .probe3(o_axi_awaddr_r)
//    );
endmodule